...
function getCheer(geckoName) {
  ...
  const pool = name === "뽀또" ? cheersPpotto : name === "뀨" ? cheersGyu : cheersBoth;
...
function compositeWithGradient(sourceImage) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    dc.fillRect(new Rect(0, gradStart + gradHeight * (i / steps), w, (gradHeight / steps) + 1));
  }
  return dc.getImage();
}
...
if (posts.length > 0) {
  ...
  if (randomPost.photoUrl) {
    try {
      ...
      bgImage = compositeWithGradient(await imgReq.loadImage());
    } catch (e) {
  ...
  const msgText = textStack.addText(getCheer(geckoName));
  msgText.font         = Font.lightSystemFont(13);
